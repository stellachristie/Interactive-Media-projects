...
// 1. background image (the static houses) - Jessica
// 2. slider (CP5) -> background sky (lightning), people - Stella
// 3. lighting data -> artificial (coded) additional lights
// 4. drop down menu (CP5) -> day - Steph
// 5. knob (CP5) -> control background music - Jesslyn
...
void setup() {
  ...
  sliderX = width*0.42; //width*0.0635;
  sliderY = height*0.9;
  sliderWidth = (int)(width/2.5); //96*(1000/135);
  ...
  ppl = new float[days.length][rowsPerDay];
  //arr = new People[int(ppl[selectDate][mySlider])];

  
  //Access data value of both tables
  light = accessData(lightning, light);
  ppl = accessData(people, ppl);            //while using S's data

  //Connect arrays with the table data
  //int count = 0; // max 30 days
  //while(count < days.length){
  //int list = 0;  
  //  for (int hour = 0; hour < hours.length; hour++) { //hours 24
  //    for (int min : mins) { //mins 4
  //      int abc = 0; //max 96
  //      while (list < peopleRowCount) {
  //        String currentTime = people.getString(list, 4);
  //        String[] timePieces = split(currentTime, ":");
  //        String[] minPieces = split(timePieces[1], " ");
  //        int currHour = Integer.valueOf(timePieces[0]);
  //        int currMin = Integer.valueOf(minPieces[0]);
  //        if(currHour == hour && currMin == min){
  //            ppl[count][abc] = people.getInt(list, 1);
  //            print(ppl[count][abc]);
  //            abc++;
  //        }
  //        list++;
  ...
  //CColor c = new CColor();
  //c.setBackground(color(343, 1, 0.84));
